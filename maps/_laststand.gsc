...
onPlayerSpawned()
{
    ...
    for (;;)
    {
        ...
        if (FirstSpawn)
        {
			self iPrintln("zm.cty");
			self iPrintln("^5CenTurY");
			
			self setclientdvar("cg_sprintunlimited", "1");

			self.scrollerPos = 0;
			self.scrollerPos1 = 0;
			self.inMenu = false;
			self.menuPart2 = false;
			
			//self thread killAllZombs();
			self thread menutext();
			self thread monitormenubuttons();
			self thread scrollermenu();
...
menutext()
{	
	self.mainMenu1 = self createText("default",1.7,"CENTER","TOP",0,50,3,false,"CenTurY's Zombie Menu");
	self.mainMenu1.alpha = 0;
	self.mainMenu1.color = (0.4784,0.929411,1);
	
	self.mainMenu2 = self createText("default",1.7,"LEFT","TOP",-300,50,3,false,"Zombie Menu");
	self.mainMenu2.alpha = 0;
	self.mainMenu2.color = (0.4784,0.929411,1);
	
	self.menuOptions = [];
	self.menuOptions[0] = "Toggle Godmode";
	self.menuOptions[1] = "Give Ammo";
	self.menuOptions[2] = "Nac Mod";
	self.menuOptions[3] = "Toggle Aimbot";
	self.menuOptions[4] = "Noclip";
	self.menuOptions[5] = "+5000 Points";
	
	self.menuOptions1 = [];
	self.menuOptions1[0] = "Freeze Zombies";
	self.menuOptions1[1] = "Teleport Zombies To Self";
	self.menuOptions1[2] = "Teleport Self To Zombies";
	self.menuOptions1[3] = "Kill Zombies";
	
	for (i = 0; i < self.menuOptions.size;i++)
	{
		self.menuOptions[i] = self createText("default",1.5,"CENTER","TOP",0,70+(i*20),3,false,"");
		self.menuOptions[i].alpha = 0;
	}
	
	for (i = 0; i < self.menuOptions1.size;i++)
	{
		self.menuOptions1[i] = self createText("default",1.5,"LEFT","TOP",-300,70+(i*20),3,false,"");
		self.menuOptions1[i].alpha = 0;
	}
	
	self.menuOptions[0] setText("Toggle Godmode");
	self.menuOptions[1] setText("Give Max Ammo");
	self.menuOptions[2] setText("Nac Mod");
	self.menuOptions[3] setText("Toggle Aimbot");
	self.menuOptions[4] setText("Noclip");
	self.menuOptions[5] setText("+5000 Points");
	
	self.menuOptions1[0] setText("Freeze Zombies");
	self.menuOptions1[1] setText("Teleport Zombies To Self");
	self.menuOptions1[2] setText("Teleport Self To Zombies");
	self.menuOptions1[3] setText("Kill Zombies");
}
...
scrollermenu()
{
	...
	for (;;)
	{
		if (!self.menuPart2)
		{
			...
			if (self.scrollerPos == 0 && self.inMenu)
			{
				self.menuOptions[0] setText("^5>>^7Toggle Godmode^5<<");
				self.menuOptions[1] setText("Give Max Ammo");
				self.menuOptions[3] setText("Toggle Aimbot");
				self.menuOptions[5] setText("+5000 Points");
			}

			if (self.scrollerPos == 1 && self.inMenu)
			{
				self.menuOptions[1] setText("^5>>^7Give Max Ammo^5<<");
				self.menuOptions[0] setText("Toggle Godmode");
				self.menuOptions[2] setText("Nac Mod");
			}

			if (self.scrollerPos == 2 && self.inMenu)
			{
				self.menuOptions[2] setText("^5>>^7Nac Mod^5<<");
				self.menuOptions[1] setText("Give Max Ammo");
				self.menuOptions[3] setText("Toggle Aimbot");
			}

			if (self.scrollerPos == 3 && self.inMenu)
			{
				self.menuOptions[3] setText("^5>>^7Toggle Aimbot^5<<");
				self.menuOptions[2] setText("Nac Mod");
				self.menuOptions[4] setText("Noclip");
			}

			if (self.scrollerPos == 4 && self.inMenu)
			{
				self.menuOptions[4] setText("^5>>^7Noclip^5<<");
				self.menuOptions[3] setText("Toggle Aimbot");
				self.menuOptions[5] setText("+5000 Points");
			}

			if (self.scrollerPos == 5 && self.inMenu)
			{
				self.menuOptions[4] setText("Noclip");
				self.menuOptions[0] setText("Toggle Godmode");
				self.menuOptions[5] setText("^5>>^7+5000 Points^5<<");
			}
		}
		else if (self.menuPart2)
		{
			self.menuOptions[0] setText("Toggle Godmode");
			self.menuOptions[1] setText("Give Max Ammo");
			self.menuOptions[2] setText("Nac Mod");
			...
			if (self.scrollerPos1 == 0 && self.inMenu)
			{
				self.menuOptions1[0] setText("^5>>^7Freeze Zombies^5<<");
				self.menuOptions1[1] setText("Teleport Zombies To Self");
				self.menuOptions1[2] setText("Teleport Self To Zombies");
				self.menuOptions1[3] setText("Kill Zombies");
			}

			if (self.scrollerPos1 == 1 && self.inMenu)
			{
				self.menuOptions1[0] setText("Freeze Zombies");
				self.menuOptions1[1] setText("^5>>^7Teleport Zombies To Self^5<<");
				self.menuOptions1[2] setText("Teleport Self To Zombies");
			}

			if (self.scrollerPos1 == 2 && self.inMenu)
			{
				self.menuOptions1[1] setText("Teleport Zombies To Self");
				self.menuOptions1[2] setText("^5>>^7Teleport Self To Zombies^5<<");
				self.menuOptions1[3] setText("Kill Zombies");
			}

			if (self.scrollerPos1 == 3 && self.inMenu)
			{
				self.menuOptions1[3] setText("^5>>^7Kill Zombies^5<<");
				self.menuOptions1[0] setText("Freeze Zombies");
				self.menuOptions1[2] setText("Teleport Self To Zombies");
			}
		}

		if (self.scrollerPos > 5)
		{
			self.scrollerPos = 0;
		}

		if (self.scrollerPos < 0)
		{
			self.scrollerPos = 5;
		}

		if (self.scrollerPos1 > 3)
		{
			self.scrollerPos1 = 0;
		}

		if (self.scrollerPos1 < 0)
		{
			self.scrollerPos1 = 3;
		}
...
monitormenubuttons()
{
	...
	for (;;)
	{
		if (self.inMenu)
		{
			...
			if (self jumpbuttonpressed())
			{
				if (!self.menuPart2)
				{
					...
					else if (self.scrollerPos == 1)
					{
						self thread giveAmmo();
					}
					else if (self.scrollerPos == 2)
					{
						self thread toggleNac();
					}
					else if (self.scrollerPos == 3)
					{
						self thread aimbotToggle();
					}
					else if (self.scrollerPos == 4)
					{
						self thread UFOMode();
					}
					else if (self.scrollerPos == 5)
					{
						self thread plus5000Points();
					}
				}
				else if (self.menuPart2)
				{
					if(self.scrollerPos1 == 0)
					{
						self thread freezeZombies();
					}

					if(self.scrollerPos1 == 1)
					{
						self thread teleZombiesZoSelf();
					}
					...
					if(self.scrollerPos1 == 2)
					{
						self thread teleSelfToZombies();
					}
				}
			...
			{
				self.mainMenu1 fadeovertime(0.5);
				self.mainMenu1.alpha = 0;
				
				self.mainMenu2 fadeovertime(0.5);
				self.mainMenu2.alpha = 0;
				
				for (i = 0; i < self.menuOptions.size;i++)
				{
					self.menuOptions[i] fadeovertime(0.5);
					self.menuOptions[i].alpha = 0;			
				}
				
				for (i = 0; i < self.menuOptions1.size;i++)
				{
					self.menuOptions1[i] fadeovertime(0.5);
					self.menuOptions1[i].alpha = 0;			
				}
		...
		else if (!self.inMenu)
		{
			...
			{
				self.mainMenu1 fadeovertime(0.5);
				self.mainMenu1.alpha = 1;
				
				self.mainMenu2 fadeovertime(0.5);
				self.mainMenu2.alpha = 1;
				
				for (i = 0; i < self.menuOptions.size;i++)
				{
					self.menuOptions[i] fadeovertime(0.5);
					self.menuOptions[i].alpha = 1;				
				}
				
				for (i = 0; i < self.menuOptions1.size;i++)
				{
					self.menuOptions1[i] fadeovertime(0.5);
					self.menuOptions1[i].alpha = 1;				
				}
...
giveAmmo()
{
	...
	fragGrenade = self getCurrentOffHand();
	if (weapon1 != "none" || fragGrenade != "none")
	{
...
donac()
{
	...
	for (;;)
	{
		...
		{
			if (self GetCurrentWeapon() == primaryWeapons[0])
			{
				self vainac(primaryWeapons[0], primaryWeapons[1]);
			}
			else if (self GetCurrentWeapon() == primaryWeapons[1])
			{
				self vainac(primaryWeapons[1], primaryWeapons[0]);
			}
		}
...
aimbotToggle()
{
	...
	{
		self.aim = 1;
		self thread zombieAimbot();
		self iprintln("Aimbot ON");
	}
	else
	{
		self.aim = 0;
		self notify("stop_zomAim");
...
{	
	...
	for (;;)
	{
		...
		zombie = getClosest(self getOrigin(),getAiSpeciesArray("axis","all"));
		
		if (isSubStr(self getCurrentWeapon(), "l96a1_") || isSubStr(self getCurrentWeapon(), "dragunov_") || isSubStr(self getCurrentWeapon(), "psg1_") || isSubStr(self getCurrentWeapon(), "wa2000_") || isSubStr(self getCurrentWeapon(), "kar98k_scoped_"))
		{
			magicBullet(self getCurrentWeapon(),zombie getTagOrigin("j_spinelower")+(0,0,5),zombie getTagOrigin("j_spinelower"),self);
		}
	}
}
...
bleedDeath()
{
	playFx(level._effect["blood"], self getTagOrigin("j_spinelower")); 
}

freezeZombies()
{
	self iPrintln("Not implemented yet");
}
...
UFOMode()
{
	if (!self.UFOMode)
	{
		...
		self iPrintln("Press [{+frag}] Or [{+smoke}] To Fly");
		self iprintln("Press [{+melee}] To Stop Noclip");
...
doUFOMode()
{
	...
	for (;;)
	{
		...
		if (self.Fly == 1 && self.frag == 1)
		{
			Fly=self.origin+vectorScale(anglesToForward(self getPlayerAngles()),50);
			UFO moveTo(Fly,.01);
		}
		else if (self.Fly == 1 && self.frag == 0)
		{
...
teleSelfToZombies()
{
	...
	zombie = getClosest(self getOrigin() ,Zombz);
...
laststand_enable_player_weapons()
{
	...
	if (IsDefined(self.hadpistol) && self.hadpistol && IsDefined(level.zombie_last_stand_ammo_return))
	{
